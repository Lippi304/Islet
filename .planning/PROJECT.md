...
v1.9 (Clipboard History) shipped 2026-07-23 — v1.10 (Live Activities Suite) started the same day from a brainstorming session (see below). v1.4 (Architecture Redesign) formally shipped 2026-07-28 (both outstanding verification items confirmed). v1.5 (Home Focus & Widget Redesign) and v1.7 (Interaction & Calendar Polish) remain open in parallel and are candidates to pick back up: v1.5 only needs Phase 33's on-device UAT to close, v1.7 is paused mid-milestone (Phase 49 Favorite/Like spike aborted after weak on-device results, Phase 50 undecided; Phase 46 Calendar Quick-Add Improvements also still open). Other standing candidates for a future milestone: a countdown timer (folded into v1.10's Timer/Pomodoro activity), gesture-based swipe navigation, Animation Speed presets (ARCH-P1), alternate app icon variants (still Out of Scope below until picked up), and Phase 49/50's Favorite/Like feature decision.
...
**Status:** Started 2026-07-23, from a brainstorming session, while v1.5 and v1.7 remain open in parallel (explicit user decision; v1.4 shipped 2026-07-28).
...
---
*Last updated: 2026-07-28 — v1.4 (Architecture Redesign) formally shipped: both outstanding verification items confirmed (`28-HUMAN-UAT.md`'s CR-01/CR-02 on-device re-tests, plus a since-discovered `27-VERIFICATION.md` Cmd-U gap for `ActivitySettingsTests`/`DiagnosticReportTests` — both suites passed clean). Tagged `v1.4`. Added the missing Phase 24 (Drag-In, SHELF-01/02) entry to Validated below — it had shipped but was never recorded. Two unrelated test failures incidentally found during the Cmd-U confirmation (`LicenseStateTests`, `SettingsViewTests` card-count drift) logged as pending todos, not yet investigated. Phase 66 (Menübar-Overflow, v1.10) also paused this session after a third consecutive NO-GO — even real Ice.app's own mechanism stopped working on this machine; still wanted, no revisit trigger. Phase 65 (Quick Actions Bar, QACTION-01/02/03) shipped within the still-open v1.10 milestone: configurable 8-slot quick-action row, on-device UAT found/fixed 2 real DND-toggle bugs (missing self-requested Focus authorization, one-way-not-toggling Shortcut), post-execution code review found/fixed 1 critical click-swallowing dead-zone. v1.10 (Live Activities Suite) started 2026-07-23 from a brainstorming session: 9 new Live Activities/HUDs (Meeting, Download-Progress, Timer/Pomodoro, Quick Notes+Obsidian, Quick Actions bar ✓, Menübar-Overflow ⏸ paused, Caps Lock, Update restyle, Coding-Progress) plus a Droppy-style Settings grid overview. v1.9 (Clipboard History) shipped and archived to `.planning/milestones/v1.9-ROADMAP.md`/`.planning/milestones/v1.9-REQUIREMENTS.md` (Phases 55-58, 7/7 requirements). v1.5 (Home Focus & Widget Redesign) and v1.7 (Interaction & Calendar Polish) remain open in parallel (explicit user decision) — v1.5 only needs Phase 33's on-device UAT to close, v1.7 is paused at Phase 49 (Favorite/Like spike aborted, Phase 50 undecided) with Phases 43-48 shipped/code-complete and Phase 46's CALVIEW-06/07 still pending. v1.8 (Settings Redesign & Island Navigation) shipped 2026-07-21, archived to `.planning/milestones/v1.8-ROADMAP.md`/`.planning/milestones/v1.8-REQUIREMENTS.md`.*
