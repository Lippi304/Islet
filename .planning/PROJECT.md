...
**Status:** Started 2026-07-30. Roadmap defined (4 phases mapped to 8 requirements, see `.planning/ROADMAP.md`). **Phase 71 (Island Corner Rounding, SHAPE-02/SHAPE-03) shipped 2026-07-30** — wing-state HUD corner radii bumped 12/6 → 16/8 across all `wingsShape()`/`mediaWingsOrToast()`/`resumePreviewWings()` call sites, plus a DEBUG-only "Corner Radius" Wing Tuner axis. On-device UAT surfaced and fixed two real app freezes (content painting outside the notch silhouette once radii grew; an unclamped corner-radius nudge producing a self-intersecting shape) and a code-review Critical finding (the freeze-fix's clamp only covered one geometry axis — resolved by moving the clamp into `NotchShape.path(in:)` itself so it protects every caller against both axes). **Phase 72 (Calendar Redesign, CALVIEW-08/CALVIEW-09) shipped 2026-07-31** — two-column expanded Calendar view (month grid left, agenda right), red today/selected badge swap, weekday header row, real EventKit update/delete wired through `CalendarService`. On-device UAT drove a live scope revision: the originally-planned whole-month day-grouped agenda (D-01/D-02) was reverted back to single-day (today/selected only) after the user found it didn't feel right live, and 5 new hover affordances were added instead (D-13 Month/Year picker popover, white hover rings/borders on day cells, chevrons, event rows, and the Add button) — see `72-CONTEXT.md` for the full supersession record. Code review found 3 non-blocking warnings (stale month-fetch race, missing end>start validation on event edit, empty-id `ForEach` collision risk) tracked in `72-REVIEW.md`. **Phase 72.1 (Volume/Brightness Bar Redesign, inserted after Phase 72) shipped 2026-07-31** — the OSD percent counter's old fading `.numericText()` transition was replaced with a custom `DigitRollText` move-only roll/slide, plus a DEBUG-only "OSD Bar/Counter Tuner" (D-05). On-device UAT drove 3 gap-closure rounds: the originally-planned opacity ramp (D-01/D-02/D-03 — bar dimming toward invisible at low fill) was fully reverted after the user disliked it live, the roll animation was retuned and then given a rapid-repeat coalescing window after held key-presses stalled digits mid-transition, and a code-review finding (WR-01 — digit identity anchored to the wrong end of the string, rolling the wrong digit across 9↔10/99↔100 boundaries) was fixed and user-approved. See `72.1-REVIEW.md` and `72.1-VERIFICATION.md` for the full trail. **Phase 72.1.1 (Liquid Glass Design Rework, inserted after Phase 72.1) shipped 2026-07-31** — reworked the island's Liquid Glass material from the old narrow-rim-only look into full-surface glass across every island state, and extended it to 6 inner elements (Quick Action buttons, view-switcher tabs, Calendar event cards, Weather widget, Tray file tiles, OSD volume/brightness bars), plus fixed the known transition-flicker bug via a narrowly-scoped `GlassEffectContainer`/`.glassEffectID`. On-device UAT was extensive (13 rounds on the base glass plan alone) and surfaced several real defects: an opaque `islandFill` base was silently blocking the native `.glassEffect()`'s real backdrop refraction (fixed, but a literal `Color.clear` fill hangs hover-to-expand on this macOS 27 beta hardware — shipped a near-zero 0.0001 value instead); an explicit `.frame()` on the glass layer broke the collapse↔expand size-morph animation (removed); `Glass.regular` oversaturated backdrop colors on both the island shell and later the OSD bars (both switched to `Glass.clear`). A code-review Critical finding (CR-01 — the 6 new inner-element glass call sites were missing the `materialStyle == .liquidGlass` gate that every other glass site already had, so non-Liquid-Glass users would still see native glass) was found and fixed post-UAT, then re-verified independently by the phase verifier. One known issue deferred by explicit user decision: the legacy (&lt;macOS 26) fallback path has an unresolved black-rectangle compositing artifact around expanded content — doesn't affect the native path this hardware runs. See `72.1.1-REVIEW.md`, `72.1.1-REVIEW-FIX.md`, and `72.1.1-VERIFICATION.md` for the full trail. 2 remaining target features (timer redesign, music Next Up queue) not yet started.
