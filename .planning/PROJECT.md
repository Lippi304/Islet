...
v1.10 (Live Activities Suite) shipped 2026-07-30 with known gaps (see Current State above). v1.4 (Architecture Redesign) and v1.5 (Home Focus & Widget Redesign) both formally shipped 2026-07-28 — v1.5 turned out to already be fully done (all 6 phases on-device-verified since 2026-07-15), just never formally closed. v1.7 (Interaction & Calendar Polish) remains open in parallel and is a candidate to pick back up: paused mid-milestone (Phase 49 Favorite/Like spike aborted after weak on-device results, Phase 50 undecided; Phase 46 Calendar Quick-Add Improvements also still open). The next milestone (`/gsd-new-milestone`) should decide priority among: v1.10's own 3 carried-forward gaps (Phase 64 Quick Notes gap-closure, Phase 66 Menübar-Overflow revisit, Phase 67 Coding-Progress), v1.7's remaining items, Phase 68/69 (App Switcher, Claude Session Usage — both still `[To be planned]`), and the 4 remaining queued Droppy-inspired ideas from the 2026-07-29 brainstorming session (island-corner-rounding, calendar-redesign-droppy, timer-slider-redesign, music-next-up-queue — see `.planning/seeds/`). Other standing candidates: gesture-based swipe navigation, Animation Speed presets (ARCH-P1), alternate app icon variants (still Out of Scope below until picked up), and Phase 49/50's Favorite/Like feature decision.
