...
| A3 | Drag sessions can originate and complete successfully from a `.nonactivatingPanel`/`.statusBar`-level, `canBecomeKey == false` window without requiring window activation | Common Pitfalls (implied), Open Questions (RESOLVED) | **HIGH risk if wrong** — this is the foundational assumption the entire phase rests on; SHELF-06 would need a materially different window architecture if drag sessions require activation. Mitigated (not eliminated) by the 20s `dragPinSafetyNetWorkItem` fallback, which bounds the failure mode regardless. Confirmed via the on-device manual checks already required in 21-01-PLAN.md's `<verification>` section (items 2-4) as part of the normal wave/phase gate — no separate pre-implementation spike is required (see Open Questions (RESOLVED) #1) |
...
1. **(RESOLVED) Does a drag session survive `ignoresMouseEvents` flipping to `true` on the source panel mid-drag?**
   ...
   - Resolution: This project's own existing Pattern 1 `.mouseMoved` global monitor (`NSEvent.addGlobalMonitorForEvents`, `Islet/Notch/NotchWindowController.swift` ~line 314) already proves the relevant mechanism: a GLOBAL `NSEvent` monitor observes events system-wide and is NOT gated by the source panel's own `ignoresMouseEvents`/click-through hit-test state — it has kept firing every hover cycle since Phase 2 regardless of the panel's interactivity flips. The revised plan (21-01-PLAN.md Task 3) adds a second global monitor, `dragReleaseMonitor` (`.leftMouseUp`), using the identical idiom, as the best-effort early-release signal for D-03. Since AppKit's Drag Manager independently owns and tracks an in-flight drag session at the OS level once `.onDrag`'s closure returns a non-empty `NSItemProvider`, the SOURCE window's subsequent click-through state does not interrupt or cancel the already-started session, and the global monitor observes the terminating `.leftMouseUp` regardless. Residual uncertainty about the live drag session itself (as opposed to the monitor) is fully bounded regardless by the 20s `dragPinSafetyNetWorkItem` fallback (T-21-03) — so no separate pre-implementation spike is required. The plan's `<verification>` section (item 2) still calls for an on-device confirmation of this exact behavior (drag survives the panel's interactivity flip, island releases promptly) as part of the normal wave/phase gate, not as a blocking pre-build spike.

2. **(RESOLVED) Exact visual behavior of the "empty provider" no-op drag (D-02, Pitfall 4)?**
   ...
   - Resolution: Both possible visual outcomes (the drag gesture never visually starts, or a ghost image briefly appears and then evaporates with nothing to drop) satisfy D-02's actual requirement ("nothing drops, no crash, no error dialog") — neither requires a code change. This is confirmed via the on-device manual check already present in 21-01-PLAN.md's `<verification>` section (item 4: externally delete a shelf item's backing temp file, attempt to drag, confirm no crash/no drop), not a separate blocker or spike.
...
- Architecture (drag-lifecycle pin/release): MEDIUM — grounded in the codebase's own established `pendingLockoutHide` pattern (HIGH confidence that pattern exists and works); drag-end detection reliability is resolved via a two-layer release (best-effort `dragReleaseMonitor` + guaranteed `dragPinSafetyNetWorkItem`), confirmed on-device per the plan's `<verification>` section rather than pre-execution (see Open Questions (RESOLVED), Assumptions A3/A4)
