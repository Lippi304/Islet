...
### Window sizing
- **D-05 (revised during 51-01 on-device UAT, still 2026-07-21):** The Settings window is fixed at 600×380 (still not user-resizable). Originally locked at 520×380, but live UAT showed the Appearance section's segmented Style picker ("Gradient / Solid Black / Liquid Glass") and the "Now Playing" accent row genuinely clipped at the window's real right edge at 520pt width — not a screenshot artifact, confirmed via a precise window-bounds capture. User chose the minimal fix: widen to 600pt, height and non-resizability unchanged. Each section's content still scrolls internally if it overflows vertically — that part of the original scroll fix is unaffected. Still rejected: making the window user-resizable.
