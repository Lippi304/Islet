...
- **D-05a (SUPERSEDES D-05 — user decision, on-device UAT round 1 of Plan 63-04, 2026-07-25):** **Nothing interrupts a live call.** While a `.meeting` head stands, every other incoming transient — Charging and Device included — is **dropped outright**: not preempted, not queued. D-05's "only Charging and Device outrank it" carve-out is withdrawn.
  - **Why it changed:** D-05 was planning-time reasoning. On real hardware (UAT step 10) plugging in the charger mid-call did not preempt at all — it queued behind the persistent Meeting head and the splash popped up minutes later once the call ended. The user was offered four options and chose "nothing interrupts a live call", explicitly to kill the delayed pop-up.
  - **Accepted cost:** the volume/brightness OSD also will not appear during a call. The user was warned and accepted this.
  - **Where it lives:** `TransientQueue.dropsWhileCallStands(_:)` in `Islet/Notch/IslandResolver.swift`, consulted by both `enqueue(_:)` and `preempt(_:)` — one place, so a future transient category cannot forget the rule. `updateHead(_:)` (mute-tap refresh) and `removeAll(where:)` (Settings toggle-off) are deliberately exempt. Loosening it later is a one-line change inside that predicate.
  - **Note:** D-05's *rank* placement (after `.device`, before `.focus`) is unchanged and still correct — the rank governs `resolve()`'s switch order, which is a separate mechanism from the queue's admission rule.
