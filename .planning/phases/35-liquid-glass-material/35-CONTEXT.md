...
**Revised (round 5 — post-phase-completion regression):** 2026-07-16 — round 4 was on-device UAT-approved (35-12) and the phase was marked complete, but the user's next fresh on-device look (after unrelated code-review fixes landed) showed a flat grey rim with zero color, "null liquid glass". Debug investigation (`.planning/debug/resolved/liquid-glass-grey-rim-regression.md`) found this was not a regression in the approved code, but two previously-latent bugs the UAT screenshots never exercised (a Phase-2 `#if DEBUG` red-tint override on the collapsed pill's fill, and RGB chromatic-fringe offsets too narrow relative to round 4's tightened rim band to visually separate under `.blendMode(.screen)`) — plus a deeper architectural reassessment: the custom Metal shader was always a hand-approximation of a system material Apple now ships natively (SwiftUI `.glassEffect(_:in:)`, macOS/iOS 26+). User was shown a reference (`github.com/callstack/liquid-glass`, confirming the real native API) and chose **D-20**: use `.glassEffect(_:in:)` natively on macOS 26+ (this project's actual build/dev target), keep the entire D-01–D-19 custom shader stack as the `<macOS 26` fallback (renamed `legacyLiquidGlassEffectLayer`), unchanged. D-01–D-19 remain historically accurate for what was built and now serve as the fallback path's design record — D-20 is additive, not a retraction.
**Status:** Shipped (round 5, native glassEffect pivot) — 2026-07-16
...
### Native glassEffect Pivot — Round 5 (Post-Completion Regression — 2026-07-16)
Round 4 was UAT-approved and the phase marked complete. A fresh on-device look afterward (user: *"Das ist einfach grauer rand nix mit liquid glass also überhaupt nicht."*) triggered a debug session (`.planning/debug/resolved/liquid-glass-grey-rim-regression.md`) that found two latent bugs the UAT process never exercised (collapsed-pill DEBUG red-tint override; RGB fringe offsets too narrow for round 4's tightened rim band) — but also surfaced that Apple ships the real Liquid Glass material natively as of macOS/iOS 26 (`SwiftUI.glassEffect(_:in:)`), which this project's own build/dev machine already runs.

- **D-20:** Use the native `.glassEffect(_:in:)` modifier on `macOS 26.0+` for the island's Liquid Glass rendering — real system-rendered glass — instead of continuing to hand-tune the custom Metal shader stack. Below macOS 26.0, the entire D-01–D-19 custom shader implementation (warp distortion, frost layer, masked chromatic fringe, rim mask) is preserved UNCHANGED as `legacyLiquidGlassEffectLayer`, gated via `#available(macOS 26.0, *)`. Chosen over (a) bumping the project's deployment target to macOS 26-only (would exclude all pre-26 users, a bigger scope change than this bug warranted) and (b) continuing to tune shader parameters (proven fragile — 4 UAT rounds plus 2 debug-session rounds on the same class of symptom before this pivot). D-01–D-19 are not retracted; they remain the accurate design record for the fallback path.
- **D-20a (same-day follow-up):** The first D-20 implementation applied `.glassEffect()` across the WHOLE island shape, tinted dark — user rejected: *"Ja es soll nur der rand so sein minimal aber nicht alles so."* (only the rim should be like that, minimal, not the whole thing). Fixed by confining the native glass to a `LiquidGlassRimRingShape` (base shape's outline stroked at 2x the legacy shader's rim-band width, intersected with the base shape) — same narrow-rim-only contract as D-12/D-13/D-14, just achieved via a ring `Shape` passed to `.glassEffect(_:in:)` instead of a shader falloff. The underlying solid dark `islandFill` base (already present at every call site, unchanged) shows through the untouched center. User-confirmed on-device.
