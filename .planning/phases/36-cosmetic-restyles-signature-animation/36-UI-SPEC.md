...
| Skiper UI (`skiper25`, reference only) | Equalizer bar visual/motion values (source ported to native SwiftUI, not installed as a package) | **Open item, non-blocking for this spec but must close before ship**: the free tier's own license header requires visible attribution to Skiper UI unless a Pro license is held. No attribution mechanism exists in Islet's UI today (per `36-CONTEXT.md` Claude's Discretion). Recommend: a one-line credit in the About/Settings screen ("Bar visualizer style inspired by Skiper UI") — small addition, surface for user decision during `/gsd:plan-phase 36`. |
