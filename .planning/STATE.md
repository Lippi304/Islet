...
Last activity: 2026-08-01 - Completed quick task 260801-0br: Two manual Live-Activity tuners (resume-preview resolution-aware conversion + Wing Tuner Height axis)
...
| 260731-61m | Phase 72.1 UAT gap closure: reverted `OSDLevelBar`'s opacity ramp (D-01/D-02/D-03) back to always-full-opacity after user disliked the dimming-toward-invisible effect on real hardware — deleted all now-dead opacity-nudge plumbing (AppStorage keys, DEBUG menu items, tests) across `ActivitySettings.swift`/`NotchPillView.swift`/`AppDelegate.swift`/`ActivitySettingsTests.swift`, kept Roll Speed ± + Reset/Print (trimmed to roll-speed-only); retuned `DigitRollText`'s base `rollResponse` 0.15→0.30 for a smoother/slower glide per user feedback, transition stays strictly `.move(edge:)`-based (D-04a no-fade preserved). Follow-up (2026-07-31, `6a24409`, direct fix): round-2 UAT found held/repeated key presses fired value changes faster than the roll animation could finish, permanently stalling a digit mid-transition — added a 120ms rapid-repeat coalescing window (`DigitRollText`'s `displayValue`/`lastChangeTime` + `onChange`) so bursts snap instantly with no animation and only a change after a pause gets the spring roll; D-04a preserved. | 2026-07-31 | 3729334 | Baseline in — pending on-device re-verification (72.1-03 checkpoint) | [260731-61m-revert-osd-bar-opacity-ramp-to-always-fu](./quick/260731-61m-revert-osd-bar-opacity-ramp-to-always-fu/) |
| 260801-0br | Two manual Live-Activity tuners: (1) converted `resumePreviewWings` (idle-hover "resume last track" preview) from a hardcoded 290×32pt/`Spacer()` layout to the shared resolution-aware `wingsShape` formula every sibling wing already uses — the one wing never converted, so it never recomputed on resolution change and was disconnected from Settings Width/Depth sliders and Wing Tuner nudges; (2) added a 6th Wing Tuner nudge axis, `heightNudge` (additive, vertical-only, wired into `wingsShape`'s shared `size.height` + `mediaWingsOrToast`'s own inline calc) — covers every Live Activity wing in one change. 3 on-device UAT rounds: found+reset an unrelated stale `marginNudge=-60` leftover from an older session (was making every wing look broken/clipped after the user's resolution change — not caused by this task); reused `mediaWingContentWidth()`'s `eqBarsWidth` for the play-icon slot instead of an oversized 160pt estimate (was hiding the icon); split the wing's tap target — whole-wing tap now falls to the normal `onClick()`/expand like every sibling wing, only the play-icon/failure-text slot has its own `onResumeTap` tap target (supersedes the original D-01 assumption per live UAT); baked in a final `-8` trailing delta from live tuning, scoped to this wing only. | 2026-08-01 | 1b85139 | Complete ✓ (on-device approved — "Gut passt") | [260801-0br-zwei-manuelle-tuner-optionen-fuer-live-a](./quick/260801-0br-zwei-manuelle-tuner-optionen-fuer-live-a/) |
