---
status: deferred
trigger: "fullscreen-enter-flash: 1-frame island flash at end of fullscreen-enter transition (06-UAT.md Test 5)"
...
Investigation concluded, not fixed — accepted as a known, product-deferred cosmetic issue. Root cause is a window-server compositor pass on the fullscreen Space-switch that happens BEFORE our reactive `orderOut` notification arrives; a 0.2s show-debounce was tried (`cc7f3c1`) and reverted (`f706f66`) since the blip is never on the app's side to pre-empt. Confirmed pre-existing since Phase 2 (02-04), NOT a Phase 6 regression. Carries forward as accepted technical debt, not blocking v1.0.
