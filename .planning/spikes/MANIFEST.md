...
| 005 | accessibility-api-queue-scraping | standard | Given Music.app's "Playing Next" panel or Spotify's Queue view is open on screen, when queried via the macOS Accessibility API (AXUIElement), then the next 5 upcoming track titles/artists are extractable as UI text nodes | PENDING | music, spotify, accessibility, phase-74, frontier |
