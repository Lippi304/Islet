...
- Spotify Desktop runs a real local JSON-RPC IPC surface on `127.0.0.1:7768` (confirmed live — it returns structured JSON-RPC errors, not silence), but its binary framing and method surface are completely undocumented with no independent reference implementation to verify against (unlike Spike 003's MediaRemote). Decoding it is a standalone reverse-engineering project, ruled out of scope by this project's own no-blind-fishing convention (Spike 006). Not a viable near-term path; the official Web API (blocked on Spotify Premium, Spike 002) remains the better-evidenced route if that gate lifts.
...
| 006 | spotify-local-connect-endpoint | standard | Given Spotify desktop is running, when Islet probes Spotify's local Connect/Zeroconf discovery endpoint (127.0.0.1), then track or queue data is inspectable without OAuth or a Premium-gated developer app | ✗ INVALIDATED | spotify, local-api, phase-74, frontier |
